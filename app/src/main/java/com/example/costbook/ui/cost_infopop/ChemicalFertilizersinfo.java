...
public class ChemicalFertilizersinfo extends Activity {
    ...
    protected void onCreate(@Nullable Bundle savedInstanceState) {
        ...
        setContentView(R.layout.chemicalfertilizersinfo);
        close = findViewById(R.id.PopupInfoClose);
        cost=findViewById(R.id.costchemicalfertilizers);
        ButterKnife.bind(this);
        gardenıd=getIntent().getIntExtra("gardenID",0);
        productıd=getIntent().getIntExtra("productID",0);
        windowSize();
        getFuelCostMethod(productıd,gardenıd);
    ...
    private void windowSize() {
        ConstraintLayout constraintLayout = findViewById(R.id.chemicalfertilizerscostinfo);
        ViewGroup.LayoutParams LayoutParams = constraintLayout.getLayoutParams();
        getWindow().setLayout(LayoutParams.width, LayoutParams.height);
        getWindow().setBackgroundDrawableResource(R.drawable.pop_up_frame);
    }

    private void getFuelCostMethod(int productID, int gardenID) {
        ...
        Call<Object> call = apiService.getChemicalfertilizersCost(productID,gardenID);
